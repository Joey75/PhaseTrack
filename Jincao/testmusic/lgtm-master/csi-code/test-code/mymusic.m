...
if nargin<3,  phs=[-90:90]*(pi/180);  end
tao=[0:3000]*(0.01);
if nargin<2,  d_norm=0.1;   end
...
for i=1:length(phs)
    for k=1:length(tao)
       ...
       b=exp(-2j*pi*tao(k)/300*(40/30)*(0:14));
       %b=exp(2j*pi*tao(k)/300)*exp(2j*pi*tao(k)/300*(40/29)*(0:14));
       c = exp(-2j*pi*d_norm*sin(phs(i))*(5.785 * 10/3)*(0:1)); %d Steering vector
       a=b.'*c;
       a= reshape(a,[30 1]);
