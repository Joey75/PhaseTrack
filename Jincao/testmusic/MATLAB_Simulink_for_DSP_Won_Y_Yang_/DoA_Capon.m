function P=DoA_Capon(R,d_norm,phs)
% Capon's MV/MVDR/ML DoA estimatin method
% Input : R      = MxM correlation matrix of a received signal
%         d_norm = Normalized antenna spacing (d/lambda)
%         phs    = Range of Azimuth angle phi[rad]
% Output: P      = Spatial spectrum vs. azimuth angle phis
...
for i=1:length(phs)
   ...
   P(i) = 1./(a'*inv(R)*a); % Eq.(7.26)
end
if nargout==0
  P_dB=10*log10(abs(P)); P_dB=P_dB-max(P_dB); plot(phs_deg,P_dB)
end
